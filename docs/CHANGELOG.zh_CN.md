...
- 新增独立萌可英语项目，按原角色顺序给 17 个场景各配一个不同萌可，每场景使用四幅 Create image 动作图，保留原有双语讲解和按键操作。原格丽乔项目独立保留。

## Unreleased

- 新增格丽乔口语卡片：17 条课堂英语，上下翻卡、短按朗读、长按完整双语讲解，以及每张四帧的 ChatGPT Create image 格丽乔动作分镜。

- Days 页从“仅今天日程”扩展为设备本地缓存、按时间排序的最多 40 条
  导入日程；每页显示四条，上/下键整页翻页，进入时定位到今天第一条所在页。
- 新增社区发布助手要求的只读 `FAP_SCREENSHOT_V1` 串口截屏端点，仅在发布助手
  发起请求时分块返回当前 DimOS 画面，不再额外申请一张整屏缓冲。
...
- 将固件和配套 App 的用户可见品牌统一为 DimOS，同时保留 `FoloPassport` BLE 名称、
  UUID 和协议标识以兼容现有刷机工具和 App。
- 统一 Android 配套 App 与 DimOS 固件的视觉风格：使用纸白底色、黑白信息层级、
  细线框面板、紧凑区块元数据、克制的红色状态点，以及相同语言的启动图标。
- 修复系统提示音只有噪音的问题：以双声道打开 ES8311 DAC 通路、选择两个 I2S
  slot、将每个单声道 PCM 采样复制到两个 slot，并以 codec 最大输出音量播放。
- 新增原创上行开机音和下行软件关机音；在翻页模式长按确定键会播放关机音、
  熄灭屏幕并进入深睡。
- 固定 CI APK 签名，使后续构建可以直接覆盖升级，无需卸载并清除 App 数据。
- 取消手工新增日程，改为只读导入 Android 系统日历；用户可选择过去和未来的
  天数范围，App 保存该范围，并在同步时向 Passport 下发最接近今天的最多 40 条日程。
- 在 App 中用动画实时显示录音接收进度，将录音定稿为可播放的 PCM WAV 文件并
  放入公共 `音乐/DimOS` 目录，在 App 列表中提供播放与删除，同时将固件
  录音圆点改为旋转磁带轮。
...
- 参考 DayRing 的视觉语言重新设计固件启动页、录音页、Days 页和 Todo 页：使用
  ...
  针对 240×320 屏幕适配的常驻四分区底栏。
- 修复紧凑屏幕中的提示文字换行、裁切和不支持的分隔符字形，并将 Days 与 Todo
  的空状态居中显示。
- 修复固件构建，并避免长度为 60 字节的日程/Todo 标题把字符串终止符写出缓冲区。
- 新增 DimOS 同步应用（开机即进入，取代演示菜单）：翻页式多界面
  （录音 / 日程 / 任务 / 音乐），上/下键翻页、确定键单击进入页面、页内上/下操作、
  确定键双击（或长按）退回翻页模式。内置中文字库（Noto Sans CJK SC 子集）
  用于渲染日程/Todo 标题。
- 新增 Android 配套 App（`android/` 目录），实现 `ai-passport-sync` BLE 客户端：
  扫描连接设备、对时、推送导入日程与 Todo、接收录音实时流并保存到手机存储、
...
- 定义 `ai-passport-sync` BLE 服务（GATT TX 通知 / RX 写入）及帧协议：对时、
  导入日程、Todo、勾选回传、录音上传、正在播放、封面、进度和状态推送。协议文档见
...
- 新增可主机测试的纯 C 核心并配套测试：`pager_core`（翻页状态机）、
  ...
  本地时间换算）。`tools/validate.sh --static` 会运行全部主机测试。
